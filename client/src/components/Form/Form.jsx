...
export default function Form({ currentId, setCurrentId }){
  ...
  return (
    <Paper className={classes.paper} style={{ filter: `invert('100%')` }} elevation={6}>
      <form autoComplete="off" noValidate className={`${classes.root} ${classes.form}`} onSubmit={handleSubmit}>
        <Typography variant="h6">{currentId ?`Editing "${post?.title}"` :'You wanna write a post?' }</Typography>
        ...
        <div style={{ padding: '5px 0', width: '94%' }}>
          <ChipInput name="tags" variant="outlined" label="Hashtags" fullWidth value={postData?.tags} onAdd={(chip) => handleAddChip(chip)} onDelete={(chip) => handleDeleteChip(chip)}/>
        </div>
